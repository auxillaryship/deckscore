...
export default function App() {
    ...
    if (selected.length === 8) {
      ...
      for (let i = 0; i < selected.length; i++) {
        ...
        for (const candidate of cards) {
          ...
          if (s && s.final - base >= 8) suggs.push({ slot: i, from, to: candidate, delta: s.final - base, score: s.final });
        }
      }
  ...
  function toggleCard(card) {
    ...
    if (idx >= 0) { setSelected((prev) => prev.filter((_, i) => i !== idx)); return; }
    if (selected.length >= 8) return;
    setSelected((prev) => [...prev, card]);
  }
  function removeSlot(i) { setSelected((prev) => prev.filter((_, idx) => idx !== i)); }
  function applySuggestion(s) { setSelected((prev) => prev.map((c, i) => (i === s.slot ? s.to : c))); }
  function quickFillRandom() { const pick = [...cards].sort(() => Math.random() - 0.5).slice(0, 8); setSelected(pick); }
  function clearAll() { setSelected([]); setResult(null); setSuggestions([]); }
  function copyShare() { const code = selected.map((c) => c.id).join(","); const u = `${window.location.origin}${window.location.pathname}?deck=${encodeURIComponent(code)}`; navigator.clipboard.writeText(u).then(()=>{ setCopied(true); setTimeout(()=>setCopied(false),2000); }); }
  function downloadDeck() { const data = JSON.stringify(selected.map((c) => c.id), null, 2); const blob = new Blob([data], { type: "application/json" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "deck.json"; a.click(); URL.revokeObjectURL(url); }
      ...
      <div className="max-w-6xl mx-auto">
        <header className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={copyShare} className="bg-amber-500 text-slate-900 px-3 py-2 rounded-md flex items-center gap-2 text-sm">{copied ? "Link Copied" : <>Share <Share2 size={14} /></>}</button>
            <button onClick={downloadDeck} className="bg-slate-700 hover:bg-slate-600 px-3 py-2 rounded-md text-sm flex items-center gap-2"><Download size={14} /> Download</button>
          </div>
        </header>
        ...
        <main className="grid grid-cols-1 md:grid-cols-12 gap-6">
          ...
          <section className="md:col-span-7 bg-slate-800/40 rounded-2xl p-4 shadow">
            <div className="relative mb-3">
              <input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder="Search cards or ids..." className="w-full bg-transparent border border-slate-700 rounded-md px-3 py-2 placeholder-slate-400" />
              <div className="absolute right-3 top-2.5 text-slate-400"><Search size={16} /></div>
            </div>

            <div className="text-sm text-slate-300 mb-3">Tap cards to add to your deck — selected cards show order badges.</div>

            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
              {filtered.map((card) => {
                const idx = selected.findIndex((c) => c.id === card.id);
                const selectedClass = idx >= 0 ? "ring-2 ring-amber-400 bg-amber-900/10" : "hover:bg-white/3";
                return (
                  <motion.button key={card.id} onClick={()=>toggleCard(card)} whileTap={{ scale:0.98 }} initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} className={`relative p-3 rounded-xl border border-slate-700 text-left ${selectedClass} transition-colors overflow-hidden`} title={`${card.name} — ${card.elixir} elixir`}>
                    <div className="flex items-center gap-3">
                      {/* clean placeholder: small elixir badge + initial */}
                      <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-slate-700 to-slate-600 flex flex-col items-center justify-center text-sm font-semibold text-amber-300">
                        <div className="text-xs">{card.elixir ?? "-"}</div>
                        <div className="text-xs text-slate-200 mt-1">{(card.name[0]||"").toUpperCase()}</div>
                      </div>

                      <div className="flex-1 min-w-0">
                        <div className="font-medium leading-tight truncate">{card.name}</div>
                        <div className="text-xs text-slate-400 mt-1 truncate">{card.type} • {(card.tags||[]).slice(0,3).join(', ')}</div>
                      </div>

                      <div className="text-xs px-2 py-1 rounded-md bg-slate-700 text-slate-200">{card.elixir ?? "—"}</div>
                    </div>
            ...
            <div className="mt-4">
              ...
              <div className="mt-3 flex gap-2 flex-wrap">
                {selected.map((c,i)=>(
                  <motion.div key={c.id} layout initial={{scale:0.95,opacity:0.9}} animate={{scale:1,opacity:1}} className="flex items-center gap-2 bg-slate-700 px-3 py-2 rounded-md">
                    <div className="w-8 h-8 rounded bg-slate-600 flex items-center justify-center text-sm font-medium text-amber-300">{c.elixir ?? "-"}</div>
                    <div className="text-sm font-medium truncate" style={{maxWidth:160}}>{c.name}</div>
          ...
          <aside className="md:col-span-5">
            <div className="sticky top-6 bg-gradient-to-b from-slate-800/40 to-slate-700/30 rounded-2xl p-5 shadow-lg">
              ...
              <div className="mt-5">
                ...
                ) : result ? (<div className="text-slate-400 text-sm">No big swap suggestions. Deck looks balanced — try replacing weak cards to improve.</div>) : null}
              </div>
