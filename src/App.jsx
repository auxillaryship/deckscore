...
function computeSynergy(deck) {
  ...
  for (let i = 0; i < deck.length; i++) {
    for (let j = i + 1; j < deck.length; j++) {
      ...
      if (
        (a.tags || []).includes("evolution") ||
        (b.tags || []).includes("evolution")
      )
        synergy += 0.18;
...
function computeOffenseDefense(deck) {
  ...
  for (const c of deck) {
    ...
    if (c.type === "troop") {
      if (tags.includes("tank") || tags.includes("heavy")) {
        off += 0.65;
        def += 0.4;
      }
    ...
    if (tags.includes("spawn") || tags.includes("split")) {
      off += 0.15;
      def += 0.15;
    }
  }
...
function RadarChart({ values, size = 160 }) {
  ...
  return (
    <div className="relative w-[200px] h-[240px] flex flex-col items-center">
      {/* precise cardinal labels */}
      <div
        style={{ left: "50%", transform: "translateX(-50%)" }}
        className="absolute top-2 text-xs text-slate-300 pointer-events-none"
      >
        Offense
      </div>
      <div
        style={{ top: "50%", transform: "translateY(-50%)" }}
        className="absolute right-2 text-xs text-slate-300 pointer-events-none"
      >
        Defense
      </div>
      <div
        style={{ left: "50%", transform: "translateX(-50%)" }}
        className="absolute bottom-6 text-xs text-slate-300 pointer-events-none"
      >
        Synergy
      </div>
      <div
        style={{ top: "50%", transform: "translateY(-50%)" }}
        className="absolute left-2 text-xs text-slate-300 pointer-events-none"
      >
        Cycle
      </div>
...
export default function App() {
  ...
  const filledDeck = useMemo(
    () => deckSlots.filter(Boolean),
    [deckSlots]
  );
  const avg = useMemo(() => avgElixir(filledDeck), [filledDeck]);
  const sigma = useMemo(
    () => sigmaElixir(filledDeck, avg),
    [filledDeck, avg]
  );
  const synergy = useMemo(
    () => computeSynergy(filledDeck),
    [filledDeck]
  );
  const winCard = useMemo(
    () => detectWinCondition(filledDeck),
    [filledDeck]
  );
  const { offense, defense } = useMemo(
    () => computeOffenseDefense(filledDeck),
    [filledDeck]
  );
  const cycle = useMemo(
    () => computeCycleScore(avg, sigma),
    [avg, sigma]
  );
  ...
  function shareLink() {
    const code = deckSlots
      .map((c) => (c ? c.id : ""))
      .join(",");
    const url = `${window.location.origin}${window.location.pathname}?deck=${encodeURIComponent(
      code
    )}`;
    if (navigator.share) {
      navigator
        .share({
          title: "DeckScore — my deck",
          text: "Check my Clash Royale deck",
          url,
        })
        .catch(() => {});
    } else {
      ...
      <div className="max-w-xl mx-auto">
        ...
        <div id="deck-export-root" className="rounded-lg">
            ...
            <div className="grid grid-cols-4 gap-2 mb-2">
              {deckSlots.slice(0, 4).map((c, i) => (
                <div
                  ...
                  className={`relative w-full h-28 rounded-lg p-3 flex flex-col justify-between text-sm cursor-pointer ${
                    c
                      ? "bg-gradient-to-br from-amber-900/10 to-amber-700/10 border border-amber-300 shadow-md overflow-visible"
          ...
          <div className="mb-4">
            <button
              onClick={() => openModalForSlot(null)}
              className="w-full bg-slate-800/40 border border-slate-700 rounded-md px-3 py-2 flex items-center gap-3"
            >
              <Search
                size={16}
          ...
          <section className="bg-slate-800/40 rounded-2xl p-4 shadow-lg mb-8">
            <div className="flex flex-col items-center">
              <RadarChart
                values={{ offense, defense, synergy, cycle }}
                size={160}
              />
              ...
              <div className="mt-8 text-center">
                ...
                <div className="mt-2 text-xs text-slate-400">
                  ...
                  <span className="font-semibold text-amber-300">
                    {filledDeck.length ? avg.toFixed(2) : "--"}
                  </span>
                </div>
              </div>
              ...
              <div className="mt-4 w-full grid grid-cols-3 gap-2">
                <div className="p-3 bg-slate-900/30 rounded text-center">
                  ...
                  <div className="mt-1 font-medium">
                    {winCard ? winCard.name : "None detected"}
                  </div>
                </div>
                <div className="p-3 bg-slate-900/30 rounded text-center">
                  <div className="text-xs text-slate-400">
              ...
              <div className="mt-4 w-full">
                <button
                  onClick={shareLink}
                  className="w-full bg-cyan-500 text-slate-900 px-3 py-2 rounded-md flex items-center justify-center gap-2"
                >
            ...
            >
              <div className="max-w-xl mx-auto h-full bg-slate-900 rounded-t-2xl shadow-xl p-4 overflow-hidden flex flex-col">
                ...
                <div className="mt-3 text-xs text-slate-400 text-center">
                  Pick cards until your deck has 8 cards. Tap a slot to
                  place the next card there.
                </div>
              </div>
            </motion.div>
          </>
